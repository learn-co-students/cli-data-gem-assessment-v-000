class TopGames::Game
  
   attr_accessor :name, :release_date, :summary
  
 def self.ranking
   #scrape pcgamer
   self.scrape_games

   end 
  
  def self.summary
    self.scrape_summary
  end 
  
  
   def self.scrape_games
     games = []
     
     games << self.scrape_kingdom
     
   
    end 
  
  def self.scrape_summary
    summary = [] 
    
    summary << self.scrape_story
    
  end
  
  def self.scrape_kingdom 
    doc = Nokogiri::HTML(open("https://www.pcgamer.com/best-rpgs-of-all-time/"))
    game_1 = self.new 
    game_1.name = doc.search("h3#kingdom-come-deliverance").text
    game_1.release_date = doc.search('p')[7].text
    game_1 
  end 
  
  def self.scrape_divinity
    doc = Nokogiri::HTML(open("https://www.pcgamer.com/best-rpgs-of-all-time/"))
    game_2 = self.new 
    game_2.name = doc.search("h3#divinity-original-sin-2").text
    game_2.release_date = doc.search('p')[7].text
    game_2
  end 
  
  def self.scrape_path
    doc = Nokogiri::HTML(open("https://www.pcgamer.com/best-rpgs-of-all-time/"))
    game_3 = self.new 
    game_3.name = doc.search("h3#path-of-exile").text
    game_3.release_date = doc.search('p')[7].text
    game_3
 end 

 def self.scrape_story
   doc = Nokogiri::HTML(open("https://www.pcgamer.com/best-rpgs-of-all-time/"))
   summary_1 = self.new 
   summary_1 = doc.search('p')[8].text #kingdom come summary 
   summary_1
  
 end 
  
  
  
end 