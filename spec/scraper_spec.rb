...
describe "Scraper" do
  ...
  describe "#scrape_portals_page" do
    it "is a class method that scrapes portal topics ('https://en.wikipedia.org/wiki/Portal:Contents/Portals') and a returns an array of hashes in which each hash represents one student" do
      ...
      expect(scraped_portals.first).to have_key(:"Culture and the arts")
      expect(scraped_portals.first).to have_key(:"Technology and applied sciences")
      expect(scraped_portals.first).to have_key(:"Philosophy and thinking")
  ...
  describe "#scrape_portal_dyk" do
    it "is a class method that scrapes a portal's Did You Know section and updates each of the @@all hash link values with an array of facts" do
    ...
    it "can handle portal pages without DYK sections" do
      profile_url = "./fixtures/student-site/students/david-kim.html"
      scraped_student = Scraper.scrape_portal_dyk
